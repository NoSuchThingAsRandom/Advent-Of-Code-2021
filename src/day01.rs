...
fn get_input(use_example_input: bool) -> anyhow::Result<Vec<usize>> {
    let filename = if use_example_input {
        EXAMPLE_INPUT_PATH.to_owned() + "01A.txt"
    } else {
...
pub fn part_1(use_example_input: bool) -> anyhow::Result<usize> {
    ...
    let mut current_depth = *AoCError::from_option(
        input.get(0),
        Some("Failed to retrieve initial depth".to_string()),
    )?;
    for depth in input {
...
pub fn part_2(use_example_input: bool) -> anyhow::Result<usize> {
    ...
    while index + 2 < input.len() {
        ...
        debug!(
            "Index {}, Values: {:?}",
            index,
            input[index..index + 2].to_vec()
        );
        if new_window > current_window {
            debug!(
                "Index {} Depth increased to {} from {}",
                index, new_window, current_window
            );
            depth_increased_count += 1;
        }
